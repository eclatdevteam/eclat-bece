...
export default function AuthPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const emailOrUsernameEntry = formData.get(role === "student" ? "username" : "email");
      const password = formData.get("password") as string;

      if (!emailOrUsernameEntry || !password) {
        toast({
      ...
      const emailOrUsername = emailOrUsernameEntry as string;

      // Validate input
      let loginEmail = emailOrUsername;
      if (role === "student") {
        loginEmail = `${emailOrUsername.trim().toLowerCase()}@student.eclat.com`;
        console.log("Auth: Attempting student login with mapped email:", loginEmail);
      } else {
      ...
      if (error) {
        console.error("Auth: Login failed for", loginEmail, ":", error);
        toast({
          title: "Login Failed",
          description: getSafeErrorMessage(error, role === "student"),
          variant: "destructive",
        });
        return;
      }

      console.log("Auth: Login successful for", loginEmail);
        ...
        <Card className="border-2 animate-scale-in">
          ...
          <CardContent>
            <Tabs defaultValue="login" className="w-full">
              <TabsList className={`grid w-full ${role === "student" ? "grid-cols-1" : "grid-cols-2"}`}>
                <TabsTrigger value="login">Login</TabsTrigger>
                {role !== "student" && <TabsTrigger value="signup">Sign Up</TabsTrigger>}
              </TabsList>
